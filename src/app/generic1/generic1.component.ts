...
//   this.SetChangeEvent(event.value, event.value.qx_code); // saby   this.selectedOption = event.value;   if
//   (typeof this.GetChangeEvent('q17a') != 'undefined' || typeof this.GetChangeEvent('q17b') != 'undefined') {
...
//   if (objIndex == -1) {
//     this.res = {
//       qx_code: event.value.qx_code,
//       qx_text: event.value.qx_text,
//       answer_text: event.value.text,
//       answer_text_score: event.value.score,
//       jumpTo: event.value.jumpTo
//     };
...
//   while (low < high) {
//     let mid = low + high >>> 1;
//     if (array[mid].qx_code < value.qx_code) 
//       low = mid + 1;
//     else 
//       high = mid;
//     }
...
// deprecated saby   if (typeof this.sourceObject.sub6 !== 'undefined') {
// this.header = this.sourceObject.header;     for (let entry of
// this.sourceObject.sub6) {       this.length = entry.options.length - 1;
// this.sub6Q1 = entry.options[0].text;       for (i = 1; i <= this.length; i++)
// {         this.sub6Q[i - 1] = entry.options[i].text;       }     }   }   if
// (typeof this.sourceObject.options !== 'undefined') {     for (let option of
...
//   }   }   if (typeof this.sourceObject.sub != 'undefined') {     for (let
// entry of this.sourceObject.sub) {       for (let option of entry.options) {
...
// this.sourceObject.sub1) {       for (let option of entry.options) {
// let selected = this.GetChangeEvent(option.qx_code);         if (typeof
// selected != 'undefined') {           if (selected.text == option.text) {
//   /* console.log(selected.qx_code); console.log(option.text);*/
// option.checked = true; this.selectedOption = selected;           } else {
//         option.checked = false;           }         }       }     }   }   if
// (typeof this.GetChangeEvent('q17a') != 'undefined' || typeof
